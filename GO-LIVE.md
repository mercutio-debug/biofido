...
Una sola scaletta, in ordine. Spunta man mano. I dettagli dei singoli pezzi
sono in `STRIPE.md` (pagamenti + Connect) e `NOTIFICHE.md` (email + push).
...
- [ ] **Resend** — account per le email transazionali.

## 1. Database — esegui le migrazioni (SQL Editor di Supabase, in ordine)
- [ ] `supabase/migrations/20260615_biofido_businesses.sql`
- [ ] `supabase/migrations/20260616_subscriptions.sql`
- [ ] `supabase/migrations/20260617_prenotazioni.sql`
- [ ] `supabase/migrations/20260618_messaggi.sql`  *(abilita anche il Realtime)*
- [ ] `supabase/migrations/20260619_push_subscriptions.sql`
- [ ] `supabase/migrations/20260620_stripe_connect.sql`

## 2. Stripe — prodotti, prezzi, Connect
- [ ] Crea 2 prodotti con 2 prezzi ricorrenti ciascuno e copia i 4 `price_id`:
...
- [ ] Il cliente apre **Le mie prenotazioni** → **Paga ora** → pagamento ok →
      stato **Pagata ✅** per entrambi; nel cruscotto Stripe vedi la fee BioFido.

---

### Passaggio in LIVE (dopo i test)
Ripeti con le chiavi **live** di Stripe (`sk_live_...`, prezzi live, nuovo
webhook + `whsec_...`), completa l'attivazione **Connect** in live e aggiorna i
