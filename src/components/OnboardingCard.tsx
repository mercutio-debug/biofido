...
import { pubblicaBozza } from "@/lib/onboarding-ai";
...
export function OnboardingCard() {
  ...
  async function approva() {
    if (!manleva) return;
    setApprovando(true);
    // se c'è una bozza AI, ora la scrivo nella scheda reale (best-effort: chi ha
    // costruito a mano non ha bozza → si salta senza bloccare l'approvazione)
    const pub = await pubblicaBozza();
    if (pub.error && !/nessun prodotto in bozza/i.test(pub.error)) {
      setApprovando(false);
      alert("Non sono riuscito a pubblicare i prodotti della bozza: " + pub.error);
      return;
    }
    ...
    setStato("completato");
    setAttivo(false);
    alert("Scheda pubblicata! I tuoi prodotti sono ora online. 🌱");
  }
  ...
  return (
    <section className="card mt-6 border-2 border-badge-yellow bg-[#fffbe9] p-6">
        ...
        <div className="mt-3 rounded-xl border-2 border-green-600 bg-white p-4">
          ...
          <label className="mt-3 flex items-start gap-2 rounded-xl bg-leaf/50 p-3 text-sm text-green-900/85">
            ...
            <span>
              ...
              BioFido</strong>, che è <strong>manlevato da qualsiasi responsabilità</strong> in
      ...
      {!inAttesa && <OnboardingAnalisiAI files={files} portale="BioFido" />}
